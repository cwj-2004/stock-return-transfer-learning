...
# 在北交所测试集上评估（特征列对齐：仅使用源域和目标域都有的特征）
X_bj_test = data['X_target_test'].copy()
y_bj_test = data['y_target_test']

# 使用源域和目标域的交集特征
common_cols = [col for col in X.columns if col in X_bj_test.columns]
if len(common_cols) < len(X.columns):
    print(f"[WARN] 目标域缺少 {len(X.columns) - len(common_cols)} 个特征")
    print(f"  缺失特征: {[col for col in X.columns if col not in X_bj_test.columns]}")
