...
# 源域数据（深市）
X_source_sz = data['X_source_sz']
y_source_sz = data['y_source_sz']
...
print(f"深市源域数据: X shape={X_source_sz.shape}")
print(f"北交所训练数据: X shape={X_target_train.shape}")

# ========================================
# 第一阶段：全局因子载荷估计（深市源域 + 目标域）
# ========================================
print("\n" + "=" * 50)
print("第一阶段：全局因子载荷估计（深市源域 + 北交所目标域）")
print("=" * 50)

# 特征对齐
source_features = X_source_sz.columns.tolist()
...
# 合并源域和目标域训练数据
X_source_aligned = X_source_sz[aligned_features].copy()
X_target_aligned = X_target_train[aligned_features].copy()

# 清洗数据
source_mask = X_source_aligned.notna().all(axis=1) & y_source_sz.notna()
X_source_clean = X_source_aligned[source_mask].reset_index(drop=True)
y_source_clean = y_source_sz[source_mask].reset_index(drop=True)
...
print(f"深市源域有效样本: {len(X_source_clean)}")
...
y_pred_global = F_global_train  # 全局模型预测即为全局因子
...
# 保存第一阶段模型和标准化器
joblib.dump(best_estimator, os.path.join(SAVE_DIR, "two_stage_sz_global_model.pkl"))
joblib.dump(combined_scaler, os.path.join(SAVE_DIR, "two_stage_sz_global_scaler.pkl"))

# 保存第二阶段模型
joblib.dump(second_stage_model, os.path.join(SAVE_DIR, "two_stage_sz_second_model.pkl"))
...
joblib.dump(two_stage_params, os.path.join(SAVE_DIR, "two_stage_sz_parameters.pkl"))
...
out_csv = os.path.join(SAVE_DIR, "two_stage_sz_predictions_oos.csv")
