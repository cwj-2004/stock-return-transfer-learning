...
X_source = data['X_source_sz']     # 深市特征
y_source = data['y_source_sz']     # 深市目标（return）
...
print(f"深市特征数: {len(source_features)}, 测试集特征数: {len(target_features)}, 对齐后特征数: {len(aligned_features)}")
...
model_path = os.path.join(SAVE_DIR, "hard_sz_elasticnet.pkl")
results_path = os.path.join(SAVE_DIR, "hard_sz_results.pkl")
...
theta_path = os.path.join(SAVE_DIR, "theta_hard_sz.pkl")
...
os.makedirs(SAVE_DIR, exist_ok=True)
out_csv = os.path.join(SAVE_DIR, "hard_sz_predictions_oos.csv")
