...
print(f"沪深合并源域数据: X shape={X_source.shape}")
print(f"北交所训练数据: X shape={X_target_train.shape}")

# ========================================
# 第一阶段：全局因子载荷估计（源域+目标域）
# ========================================
print("\n" + "=" * 50)
print("第一阶段：全局因子载荷估计（沪深源域 + 北交所目标域）")
print("=" * 50)
...
# 合并源域和目标域训练数据
X_source_aligned = X_source[aligned_features].copy()
X_target_aligned = X_target_train[aligned_features].copy()

# 清洗数据
source_mask = X_source_aligned.notna().all(axis=1) & y_source.notna()
X_source_clean = X_source_aligned[source_mask].reset_index(drop=True)
y_source_clean = y_source[source_mask].reset_index(drop=True)
...
print(f"源域有效样本: {len(X_source_clean)}")
...
# 保存第一阶段模型和标准化器
joblib.dump(best_estimator, os.path.join(SAVE_DIR, "two_stage_sh_sz_global_model.pkl"))
joblib.dump(combined_scaler, os.path.join(SAVE_DIR, "two_stage_sh_sz_global_scaler.pkl"))

# 保存第二阶段模型
joblib.dump(second_stage_model, os.path.join(SAVE_DIR, "two_stage_sh_sz_second_model.pkl"))
...
joblib.dump(two_stage_params, os.path.join(SAVE_DIR, "two_stage_sh_sz_parameters.pkl"))
...
out_csv = os.path.join(SAVE_DIR, "two_stage_sh_sz_predictions_oos.csv")
df_pred_out.to_csv(out_csv, index=False, encoding="utf-8-sig")

print(f"\n模型已保存到: {SAVE_DIR}")
print(f"预测结果已保存到: {out_csv}")
print("=" * 50)
print("两阶段估计（沪深合并版）完成！")
print("=" * 50)